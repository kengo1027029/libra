...
export function SignupForm() {
  ...
  return (
    <form onSubmit={handleSignup} className="space-y-4">
      <h1 className="text-4xl font-semibold tracking-tight text-neutral-900">サインアップ</h1>
      ...
      <div className="space-y-3 pt-2">
        <label className="relative block">
          ...
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="メールアドレス" className="w-full rounded-xl border border-neutral-300 bg-white py-3 pl-11 pr-4 text-sm text-neutral-900 placeholder:text-neutral-400 focus:border-[#2c32f1] focus:outline-none focus:ring-2 focus:ring-[#2c32f1]/20" />
        </label>
        <label className="relative block">
          <span className="pointer-events-none absolute inset-y-0 left-4 inline-flex items-center">
            <KeyIcon />
          </span>
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="パスワード" className="w-full rounded-xl border border-neutral-300 bg-white py-3 pl-11 pr-4 text-sm text-neutral-900 placeholder:text-neutral-400 focus:border-[#2c32f1] focus:outline-none focus:ring-2 focus:ring-[#2c32f1]/20" />
        </label>
      </div>
      ...
      <button type="button" onClick={handleGoogleSignup} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-neutral-300 bg-white py-3 text-sm font-semibold text-neutral-700 transition hover:bg-neutral-50">
        <GoogleIcon />
        Google
      </button>
      ...
      <p className="pt-2 text-center text-sm text-neutral-500">
        既にアカウントをお持ちの方はこちらから{" "}
        <Link href="/admin/login" className="font-semibold text-[#1767e8]">
          ログイン
        </Link>
      </p>
    </form>
